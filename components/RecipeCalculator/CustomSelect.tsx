    ...
    return (
        <Listbox value={value} onChange={onChange}>
            <div className="relative">
                <Listbox.Button className="w-full px-3 py-2.5 sm:px-4 sm:py-3.5 bg-white border-2 border-amber-300 rounded-xl text-gray-900 text-sm sm:text-base font-medium transition-all duration-200 ease-in-out cursor-pointer shadow-sm hover:border-amber-400 hover:shadow-md focus:border-amber-500 focus:ring-4 focus:ring-amber-100 focus:shadow-lg outline-none text-left pr-10 sm:pr-12">
                ...
                <Transition
                    ...
                    leaveTo="opacity-0"
                >
                    <Listbox.Options className="absolute z-[100] w-full py-1 sm:py-2 mt-1 overflow-auto text-sm sm:text-base bg-amber-100 rounded-xl shadow-lg max-h-28 sm:max-h-32 ring-1 ring-black ring-opacity-5 focus:outline-none border border-amber-200">
                        {units.map((unit) => (
                            <Listbox.Option
                                key={unit.value}
                                className={({ active }) =>
                                    `relative cursor-pointer select-none py-2 sm:py-3 px-3 sm:px-4 transition-colors duration-200 ${active ? 'bg-amber-50 text-amber-900' : 'text-gray-900'
